...
\date{This is a Technical Report on May 13, 2025, \\
    Last edited: \today}

\title{YOLO11-Earth: Training a Remote Sensing Open Vocabulary Detector at only \$21}
\author{
    Zhicheng Xu \\
...
\begin{abstract}
In the field of remote sensing images processing, conventional
object detection models faces problems such as large differences in dataset
label coverage and image resolution, and a lack of detection models with strong
transferability. Meanwhile in computer vision community, with similar concerns,
Open Vocabulary Detection (OVD) has been proposed to detect labels outside the training data.
However, existing OVD models consume a large amount of accelerator
memory during training and require multiple datasets from different tasks,
making this solution cost high. 

Aiming to bridge this gap, this technical report introduces a lightweight
solution for open vocabulary detection in remote sensing images. This solution
mixed YOLO11 and YOLOv8-Worldv2 and proposes a new variant,
YOLO11-Earth, through model pruning. It only uses the conventional xView
dataset and a consumer-grade GPU, enabling real-time open vocabulary detection capabilities on
remote sensing images. 
In the DIOR zero-shot transfer test, YOLO11-Earth outperforms the vanilla
YOLOv8-Worldv2 and is comparable to its tuned-on-xView version. In the DIOR
fine-tuning test, YOLO11-Earth surpasses YOLOv8-Worldv2 and YOLO11 when
using both partial and full fine-tuning training data, with an mAP50 of 84.5\%
for YOLO11x-Earth, demonstrating strong cross-dataset transfer
ability. YOLO11-Earth can also pass the pre-trained backbone network to
the YOLO11-seg model and be applied to the in-box segmentation task on the
Potsdam dataset.

This study also uses machine learning theory to explain the source of
YOLO11-Earth's open vocabulary detection ability. The lightweight design of
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=10cm]{../image/0.png}
    \caption{The "prompt first, then detect" pipeline of open vocabulary detection model.}
    \label{fig:"prompt first, then detect"}
\end{figure}

Conventional object detection models, such as R-CNN, SSD, and YOLO, have shown remarkable 
performance in detecting objects and classes within their training datasets \cite{girshick2014rich,liu2016ssd,redmon2016you}. 
However, these models often struggle when deployed in new scenarios with unseen 
categories, as they are inherently limited by the closed-set training \cite{akyon2022slicing,wu2022uiu,yang2022querydet}. 
To address this limitation, OVD models have been developed to extend the detection 
capabilities beyond the training dataset, enabling the identification of unseen objects 
...
In this technical report, we develop a lightweight OVD model tailored for remote sensing imagery. 
Our proposed model, YOLO11-Earth, integrates the strengths of YOLO11 and YOLOv8-Worldv2 
while significantly reducing the computational cost compared to SOTA (State of The Art) OVD models. 
This is achieved through model pruning and the use of a single, conventional dataset 
(xView) along with a consumer-grade GPU (RTX4090). 
...
real-time OVD on any remote sensing images, thereby overcoming the limitations of existing 
...
which involves reconstruct the Vision-Language Path Aggregation Network (VL-PAN) to 
enhance multimodal learning on xView. 
...
Finally, Section 6 analyzes the low cost advantages and interpretability of YOLO11-Earth, 
and discusses potential improvements and future directions. 


\section{Related Work}

\subsection{Object Detection Models}
Object detection models have evolved from two-stage methods 
like the R-CNN series, which prioritize accuracy at a high 
computational cost \cite{girshick2014rich,girshick2015fast,ren2016faster,he2017mask}, 
to one-stage models such as SSD and the YOLO family, which are 
optimized for real-time performance \cite{liu2016ssd,redmon2016you}. 
The YOLO series, in particular, has seen rapid development. 
Early versions improved network structures and multi-scale 
handling \cite{redmon2017yolo9000, redmon2018yolov3}. 
Subsequent iterations introduced innovations like advanced 
loss functions \cite{bochkovskiy2020yolov4}, anchor-free 
mechanisms \cite{li2022yolov6}, module-level reparameterization 
\cite{wang2023yolov7}, and attention mechanisms \cite{tian2025yolov12}, 
while continuously enhancing training strategies, parameter 
efficiency, and deployment flexibility \cite{jocher2020ultralytics, 
sohan2024review, wang2024yolov9, wang2024yolov10, khanam2024yolov11}.

Concurrently, the integration of Transformer architectures has 
led to end-to-end detectors like DETR \cite{carion2020end} and 
its successors, such as DINO, which improved performance through 
techniques like contrastive de-noising training \cite{zhang2022dino}. 
Further refinements in this domain have yielded models like 
RT-DETR and Mask DINO, pushing performance boundaries 
\cite{li2023mask, zhao2024detrs, lv2024rt}. 
...
follows the general schema of open vocabulary detection model construction:
...
would not be lost at slice boundaries (Figure 2). We rewrote the DOTA 
...
fixed at $800 \times 800$ pixels, saving a significant amount of local storage. 
The label files are in xml format (Pascal VOC standard format). To better 
train a YOLO series derived model, the original xml dataset was converted to txt text 
...
and mapping mask pixels to YOLO-format instance-level segmentation TXT label files  
(COCO-seg style).  
...
The experimental framework of this study is illustrated in Figure 4. We first  
...
This study uses the Ultralytics PyTorch implementation of YOLO-Worldv2 (YOLOv8-Worldv2), which is easier to deploy and train.  
...
Figure 5 shows the detailed internal parameter layers of YOLO11-Earth.

\subsubsection{VL-PAN Design for Close-Set Datasets}
...
As shown in Figure 6, after aggregating text encoding
...
\midrule
YOLOv8s-Worldv2 & 52.1 (+0.5) & 36.5 (-6.5) & 45.6 (+5.7) & 29.2 (+5.4) & 46 \\
YOLOv8m-worldv2 & 55.9 (-0.9) & 42.1 (+0.22) & 50.8 (+6) & 33.3 (+5.8) & 103.9 \\
YOLOv8l-Worldv2 & 57.9 (-0.5) & 46.1 (-1.7) & 54.1 (+5.4) & 36.3 (+5.7) & 196.6 \\
YOLOv8x-Worldv2 & 59.4 (-1.2) & 49.7 (+0) & 57.1 (+5.5) & 38.7 (+5.7) & 299.4 \\
\midrule
YOLO11s-Earth & 65.4 (+7.1) & 40.4 (-5.9) & 54.1 (+7.4) & 36.2 (+7.2) & 50.0 \\
YOLO11m-Earth & 64.2 (-1.0) & 49.4 (-1.0) & 59.2 (+5.9) & 41.0 (+6.2) & 137.4 \\
YOLO11l-Earth & 63.6 (-0.4) & 53.2 (-0.8) & 61.3 (+5.3) & 43.0 (+5.7) & 161.1 \\
YOLO11x-Earth & 73.4 (+1.8) & 58.1 (-0.9) & 69.1 (+6.2) & 51.8 (+7.2) & 344.0 \\
\midrule
YOLO11s & 68.4 (+6.2) & 42.5 (-5.5) & 56.7 (+7.8) & 38.1 (+7.6) & 21.4 \\
YOLO11m & 71.8 (+0) & 54.5 (-0.6) & 65.5 (+5.5) & 46.4 (+6.3) & 67.9 \\
YOLO11l & 71.2 (+0.3) & 57.9 (-0.1) & 67.2 (+5.7) & 47.7 (+6.5) & 86.8 \\
YOLO11x & 72.2 (-1.8) & 58.3 (+0.1) & 67.9 (+5.1) & 49.0 (+6.0) & 194.8 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Model Structure} & \textbf{P} & \textbf{R} & \textbf{mAP50} & \textbf{mAP50:95} & \textbf{GFLOPS} \\
\midrule
YOLOv8s-Worldv2 & 21.0\% & 3.75\% & 12.3\% & 7.40\% & 46 \\
YOLOv8m-Worldv2 & 26.1\% & 6.20\% & 15.9\% & 8.73\% & 103.9 \\
YOLOv8l-Worldv2 & 27.4\% & 6.87\% & 17.1\% & 9.89\% & 196.6 \\
YOLOv8x-Worldv2 & 35.2\% & 7.60\% & 21.1\% & 11.90\% & 299.4 \\
\midrule
YOLO11s-Earth & 26.7\% & 6.15\% & 16.1\% & 9.85\% & 50.0 \\
YOLO11m-Earth & 32.6\% & 7.73\% & 20.3\% & 11.60\% & 137.4 \\
YOLO11l-Earth & 34.7\% & 8.73\% & 21.5\% & 11.60\% & 161.1 \\
YOLO11x-Earth & 36.7\% & 9.31\% & 22.9\% & 13.90\% & 344.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
GLIP-FT \cite{li2022grounded,pan2025locate} & Swin-T & O365, GoldG, CC3M, SBU & 87.8 \\
GroundingDINO-FT \cite{liu2024grounding,pan2025locate}& Swin-T & O365, GoldG, Cap4M & 90.4 \\
GroundingDINO-FT \cite{liu2024grounding,pan2025locate} & Swin-T & LAE-1M & 91.1 \\
LAE-DINO-FT \cite{pan2025locate} & Swin-T & O365, GoldG, Cap4M & 92.0 \\
LAE-DINO-FT \cite{pan2025locate} & Swin-T & LAE-1M & 92.2 \\
YOLO11x-Earth (ours) & CSPNet & xView & 84.5 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Figure 9 illustrates the segmentation results of the YOLO11x-seg models with YOLO11-Earth backbone on the test set of the Potsdam dataset. 
The segmentation results were visualized by converting the bounding box predictions back to masks using RGB mapping. The model showed good 
performance in segmenting objects with relatively fixed shapes, such as trees and cars. However, for objects with more diverse shapes, such 
as impervious surfaces, sparse vegetation, and buildings, the segmentation accuracy was lower. This is partly due to the design of the Potsdam 
dataset, which includes a background class that leads to significant overlap of bounding boxes, making it challenging to achieve accurate 
...
Currently, SOTA models in the field of open vocabulary detection and open set detection, 
...
In summary, the core idea of best models in the field of open vocabulary and open set detection 
...
remote sensing imagery. Built upon the foundations of YOLO11 and YOLOv8-Worldv2, YOLO11-Earth leverages model 
